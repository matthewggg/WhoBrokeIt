...
public class CommandSubClear implements CommandExecutor {

    private HashMap<String, CommandExecutor> commandMap = new HashMap<String, CommandExecutor>();
    
    public CommandSubClear() {
        this.registerCommand("location", new CmdClearLocation());
        this.registerCommand("player", new CmdClearPlayer());
        this.registerCommand("all", new CmdClearAll());
        this.registerCommand("time", new CmdClearTime());
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
            if (args.length > 1 && hasCommand(args[0])) {
                if(sender.hasPermission("wbi.clear." + args[0])) {
